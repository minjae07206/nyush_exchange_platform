...
router.post('/', async (req:Request, res: Response) => {
    ...
    try {
        const filePath = path.join(__dirname, '..', '..', '..', 'src', 'sql_queries', 'save_post.sql');
        const save_post_query = readFileSync(filePath, 'utf-8');
        ...
        await pool.query(save_post_query, [postId, userId]);
