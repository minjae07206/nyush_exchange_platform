...
interface ArchivePopupProps {
    ...
    setShowArchivePopup: (state:boolean) => void;
}




export default function ArchivePopup({handleDeleteButtonClick, handleArchiveButtonClick, setShowArchivePopup}:ArchivePopupProps) {
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white rounded-xl shadow-lg p-6 text-center w-96">
                ...
                <Button customClass="p-1 bg-gray-600 hover:bg-gray-700" buttonText="Cancel" handleButtonClickProp={() => { setShowArchivePopup(false); }}></Button>
            </div>
        </div>
    )
}